...
using BeerO.SlackCore.Extensions;
using BeerO.SlackCore.Logging;
using BeerO.SlackCore.MessagingPipeline.Middleware;
using BeerO.SlackCore.MessagingPipeline.Request;
using BeerO.SlackCore.MessagingPipeline.Request.Extensions;
...
namespace BeerO.SlackCore
{
    internal class NoobotCore : INoobotCore
    {
        ...
        public async Task SendMessage(ResponseMessage responseMessage)
        {
            ...
            if (chatHub != null)
            {
                ...
                else
                {
                    ...
                    string textTrimmed = botMessage.Text.Length > 50
        ...
        {
            var user = this._connection.UserCache.FirstOrDefault(x =>
                x.Value.Name.Equals(username, StringComparison.OrdinalIgnoreCase));
        ...
        {
            var channel = this._connection.ConnectedChannels()
                .FirstOrDefault(x => x.Name.Equals(channelName, StringComparison.OrdinalIgnoreCase));
        ...
        {
            return this._connection.UserCache.ContainsKey(message.User.Id)
                ? this._connection.UserCache[message.User.Id].Name
                : string.Empty;
        }
        ...
        private async Task<SlackChatHub> GetUserChatHub(string userId, bool joinChannel = true)
        {
            ...
            if (this._connection.UserCache.ContainsKey(userId))
            {
                ...
                chatHub = this._connection.ConnectedDMs()
