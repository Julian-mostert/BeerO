...
namespace BeerO.SpotifyAuth
{
    ...
    public class TokenSwapWebAPIFactory
    {
        ...
        public TokenSwapWebAPIFactory(string exchangeServerUri, Scope scope = Scope.None, string hostServerUri = "http://localhost:4002", int timeout = 10, bool autoRefresh = false, bool openBrowser = true)
        {
            ...
            this.HostServerUri = hostServerUri;
            this.OpenBrowser = openBrowser;

            this.OnAccessTokenExpired += async (sender, e) =>
            {
                if (this.AutoRefresh)
                {
                    await this.RefreshAuthAsync();
                }
            };
        }
        ...
        {
            Token token = await this.lastAuth.RefreshAuthAsync(this.lastToken.RefreshToken);

            if (token == null)
            {
                this.OnAuthFailure?.Invoke(this, new AuthFailureEventArgs($"Token not returned by server."));
            }
            else if (token.HasError())
            {
                this.OnAuthFailure?.Invoke(this, new AuthFailureEventArgs($"{token.Error} {token.ErrorDescription}"));
            }
            else if (string.IsNullOrEmpty(token.AccessToken))
            {
                this.OnAuthFailure?.Invoke(this, new AuthFailureEventArgs("Token had no access token attached."));
            }
            else
            {
                this.lastWebApi.AccessToken = token.AccessToken;
                this.OnAuthSuccess?.Invoke(this, new AuthSuccessEventArgs());
            }
        }
        ...
        {
            if (this.webApiTimeoutTimer == null) return;

            // The while loop in GetWebApiSync() will react and trigger the timeout.
            this.webApiTimeoutTimer.Stop();
            this.webApiTimeoutTimer.Dispose();
            this.webApiTimeoutTimer = null;
        }
        ...
        public async Task<SpotifyWebAPI> GetWebApiAsync()
        {
            return await Task<SpotifyWebAPI>.Factory.StartNew(() =>
            {
                ...
                this.CancelGetWebApiRequest();

                this.lastAuth = new TokenSwapAuth(
                    exchangeServerUri: this.ExchangeServerUri,
                    serverUri: this.HostServerUri,
                    scope: this.Scope,
                    htmlResponse: this.HtmlResponse)
                {
                    ShowDialog = this.ShowDialog,
                    MaxGetTokenRetries = this.MaxGetTokenRetries,
                    TimeAccessExpiry = this.AutoRefresh || this.TimeAccessExpiry
                };
                this.lastAuth.AuthReceived += async (sender, response) =>
                {
                    if (!string.IsNullOrEmpty(response.Error) || string.IsNullOrEmpty(response.Code))
                    {
                        ...
                        if (!this.webApiTimeoutTimer.Enabled) return;

                        this.OnAuthFailure?.Invoke(this, new AuthFailureEventArgs(response.Error));
                        currentlyAuthorizing = false;
                        return;
                    }

                    this.lastToken = await this.lastAuth.ExchangeCodeAsync(response.Code);

                    if (this.lastToken == null || this.lastToken.HasError() || string.IsNullOrEmpty(this.lastToken.AccessToken))
                    {
                        // We only want one auth failure to be fired, if the request timed out then don't bother.
                        if (!this.webApiTimeoutTimer.Enabled) return;

                        this.OnAuthFailure?.Invoke(this, new AuthFailureEventArgs("Exchange token not returned by server."));
                        currentlyAuthorizing = false;
                        return;
                    }

                    if (this.lastWebApi != null)
                    {
                        this.lastWebApi.Dispose();
                    }

                    this.lastWebApi = new SpotifyWebAPI()
                    {
                        TokenType = this.lastToken.TokenType,
                        AccessToken = this.lastToken.AccessToken
                    };

                    this.lastAuth.Stop();

                    this.OnAuthSuccess?.Invoke(this, AuthSuccessEventArgs.Empty);
                    currentlyAuthorizing = false;
                };
                this.lastAuth.OnAccessTokenExpired += async (sender, e) =>
                {
                    if (this.TimeAccessExpiry)
                    {
                        this.OnAccessTokenExpired?.Invoke(sender, AccessTokenExpiredEventArgs.Empty);
                    }
                ...
                this.lastAuth.Start();
                this.OnExchangeReady?.Invoke(this, new ExchangeReadyEventArgs { ExchangeUri = this.lastAuth.GetUri() });
                if (this.OpenBrowser)
                {
                    this.lastAuth.OpenBrowser();
                }

                this.webApiTimeoutTimer = new System.Timers.Timer
                {
                    AutoReset = false,
                    Enabled = true,
                ...
                while (currentlyAuthorizing && this.webApiTimeoutTimer.Enabled) ;

                // If a timeout occurred
                if (this.lastWebApi == null && currentlyAuthorizing)
                {
                    this.OnAuthFailure?.Invoke(this, new AuthFailureEventArgs("Authorization request has timed out."));
                }

                return this.lastWebApi;
            });
        }
    }
}
