...
namespace BeerO.SpotifyAuth
{
    ...
    public class TokenSwapAuth : SpotifyAuthServer<AuthorizationCode>
    {
        ...
        {
            StringBuilder builder = new StringBuilder(this.exchangeServerUri);
            builder.Append("?");
        ...
        async Task<Token> GetToken(string grantType, string authorizationCode = "", string refreshToken = "", int currentRetries = 0)
        {
            ...
            try
            {
                var siteResponse = await httpClient.PostAsync(this.exchangeServerUri, content);
            ...
            if (currentRetries >= this.MaxGetTokenRetries)
            {
                return null;
            }
            else
            {
                ...
                return await this.GetToken(grantType, authorizationCode, refreshToken, currentRetries);
            }
        }
        ...
        {
            if (!this.TimeAccessExpiry) return;

            if (this.accessTokenExpireTimer != null)
            {
                this.accessTokenExpireTimer.Stop();
                this.accessTokenExpireTimer.Dispose();
            }

            this.accessTokenExpireTimer = new System.Timers.Timer
            {
                Enabled = true,
                Interval = token.ExpiresIn * 1000,
                AutoReset = false
            };
            this.accessTokenExpireTimer.Elapsed += (sender, e) => this.OnAccessTokenExpired?.Invoke(this, EventArgs.Empty);
        }
        ...
        {
            Token token = await this.GetToken("authorization_code", authorizationCode: authorizationCode);
            if (token != null && !token.HasError() && !string.IsNullOrEmpty(token.AccessToken))
            {
                this.SetAccessExpireTimer(token);
            }
            return token;
        }
        ...
        {
            Token token = await this.GetToken("refresh_token", refreshToken: refreshToken);
            if (token != null && !token.HasError() && !string.IsNullOrEmpty(token.AccessToken))
            {
                this.SetAccessExpireTimer(token);
            }
    ...
    internal class TokenSwapAuthController : WebApiController
    {
        ...
        public Task<bool> GetAuth()
        {
            ...
            return this.HttpContext.HtmlResponseAsync(((TokenSwapAuth)auth).HtmlResponse);
        }
    }
}
